...
def scrape_investing(df_stock, from_date, to_date, symbol):
    """ Function to get stock's OHLC data from investing.com """
    try:
...
def scrape_yahoo(df_stock, start, end, symbol):
    """ Function to get stock's OHLC data from yahoo.com """
    try:
...
def scrape_jitta(df_stock, symbol):
    """ Function to get stock's financial statement data from jitta.com """
    global driver
...
def get_data_to_df(driver, df):
    """ Function to scrape data table in jitta.com to df """
...
if __name__ == "__main__":
    ...
    # Scrape SET Hist Data
    for index in ['SET', 'SET TRI']:
        df = investpy.get_index_historical_data(index=index, country='thailand', from_date=from_date, to_date=to_date, as_json=False, order='ascending')
        df.drop(labels='Currency', axis=1, inplace=True)
        df[['Return']] = df[['Close']].pct_change()
        df = df.round(6)
        df.to_csv(dir_path + 'data/' + index + '.csv')
